...
struct SettingsView: View {
  ...
  @AppStorage(DownsampledImageCache.cacheLimitKey) private var cacheLimitGB: Double = 10
  ...
  var imageCacheRow: some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      HStack {
        ...
        Button("Clear cache", role: .destructive) {
          DownsampledImageCache.clearCache()
          refreshCacheSize()
        }
        .foregroundStyle(.red)
      }
    ...
    .onChange(of: cacheLimitGB) { _, _ in
      DownsampledImageCache.enforceDiskCacheLimit()
      refreshCacheSize()
    }
  ...
  private func refreshCacheSize() {
    cacheSizeBytes = DownsampledImageCache.diskCacheSizeBytes()
  }
