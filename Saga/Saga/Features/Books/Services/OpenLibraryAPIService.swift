...
struct OpenLibraryAPIService: CoverImageURLProvider {
    ...
    static func bestCoverCandidate(forISBN isbn: String, title: String, author: String) async throws -> CoverImageCandidate? {
    ...
    static func isbnFor(title: String, author: String) async throws -> String? {
        ...
        for docs in docGroups {
            let exactTitleISBNs = isbnCandidates(from: docs, normalizedTitle: normalizedTitle, requireTitleMatch: true)
            if let isbn = preferredISBN(from: exactTitleISBNs) {
                return isbn
            }
            let fallbackISBNs = isbnCandidates(from: docs, normalizedTitle: normalizedTitle, requireTitleMatch: false)
    ...
    private static func searchResponse(forTitle title: String, author: String) async throws -> SearchResponse? {
        ...
        components.queryItems = [
            ...
            URLQueryItem(
                name: "fields",
                value: "key,title,author_name,language,cover_i,cover_edition_key,editions,editions.title,editions.isbn"
            ),
    ...
    private static func bestCoverCandidate(forTitle title: String, author: String) async throws -> CoverImageCandidate? {
        let searchResponse = try await searchResponse(forTitle: title, author: author)
        guard let response = searchResponse,
              let docs = response.docs, !docs.isEmpty else {
            return nil
    ...
    private static func coverURLCandidates(from doc: Doc) -> [URL] {
        ...
        if let coverEditionKey = doc.coverEditionKey,
           let url = URL(string: "\(Constants.coverImagesBaseURL)/b/olid/\(coverEditionKey)-L.jpg?default=false") {
            urls.append(url)
        }
    ...
    private static func isbnCandidates(
        ...
        let filteredEditions = requireTitleMatch
    ...
    private static func prioritizedDocGroups(
        ...
        let authorMatchesDocs = docs.filter { authorMatches($0.authorName, normalizedAuthor: normalizedAuthor) }
        let titleMatchesDocs = docs.filter { titleMatches($0.title, normalizedTitle: normalizedTitle) }
        let authorTitleMatchesDocs = authorMatchesDocs.filter { titleMatches($0.title, normalizedTitle: normalizedTitle) }
        return [authorTitleMatchesDocs, authorMatchesDocs, titleMatchesDocs, docs].filter { !$0.isEmpty }
    }
